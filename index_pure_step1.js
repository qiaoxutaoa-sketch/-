  ...
  try {
    switch (action) {
      ...
      case 'batchConsume': {
        ...
        // 瀵逛簬姣忎竴涓嬀閫夌殑瀛︾敓锛屾墸闄よ鏃讹紝骞跺鍔犱竴鏉℃秷璇捐褰?\nfor (let i = 0; i < records.length; i++) {
          ...
          try {
            ...
            await db.collection('class_records').add({
              data: {
                ...
                comment: record.comment || '', // 鍧氬喅涓嶄娇鐢ㄩ粯璁ゆ枃鏈薄鏌撶偣璇勫瓧娈?\ncourseContent: record.courseContent || '', // 纭繚鎺堣鍐呭琚嫭绔嬪瓨鍌?\nartwork: record.artwork || '', // 鎺ユ敹鍓嶇缁熶竴涓婁紶鐨勮鍫傞閲囩収鐗?\nclassId: classId && classId !== 'temp' ? classId : '',
                timestamp: Date.now()
              }
            })

            // 鎵ｅ噺瀵瑰簲瀛︾敓鐨勫墿浣欒鏃?\n// 浣跨敤 db.command.inc 鍘熷瓙鎿嶄綔鐩存帴鍦ㄤ簯绔噺鍘昏鏃讹紝鏈€瀹夊叏
              ...
              if (stuData && stuData._openid) {
                try {
                  await cloud.openapi.subscribeMessage.send({
                    ...
                    lang: 'zh_CN',
                    data: {
                      thing1: { value: record.studentName },       // 鍋囪 thing1 鏄鍚?\nthing2: { value: record.course },            // 鍋囪 thing2 鏄绋?\nnumber3: { value: hours },                   // 鍋囪 number3 鏄墸闄よ鏃?\nnumber4: { value: stuData.remain - hours }   // 鍋囪 number4 鏄墿浣欒鏃?\n},
      ...
      case 'getMyTeachers': {
        ...
        // 2. 鏌ユ壘鎵€鏈夌殑鐝骇骞跺湪浠ｇ爜涓仛鏁扮粍姹備氦闆嗚繃婊わ紝閬垮厤浜戝紑鍙戜娇鐢?_.in 澶勭悊鍙屾暟缁勭殑灞€闄愭€?\nconst classRes = await db.collection('classes').get()
      ...
      // 13.5 瀹堕暱绔強鏁欏笀绔嚜涓婚噸缃鍛樹釜浜哄ご鍍?\ncase 'updateStudentAvatar': {
        ...
        // 閴存潈锛氬彧鏈夌粦瀹氫簡璇ュ闀跨殑瀛︾敓锛屾墠鑳借璇ュ闀夸慨鏀瑰ご鍍?\nconst stuRes = await db.collection('students').doc(studentId).get();
        if (!stuRes.data || stuRes.data._openid !== openid) {

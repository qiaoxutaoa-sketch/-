...
exports.main = async (event, context) => {
  ...
  const parentActions = ['getMyStudents', 'getMyTeachers', 'getMyRecords', 'bindExistingStudent', 'submitNewApplication', 'login', 'submitLeave', 'updateStudentAvatar', 'activateVIP', 'checkVIP'];
  ...
  try {
    switch (action) {
      ...
      // 1. 鍒犻櫎瀛﹀憳
            case 'bindExistingStudent': {
        const { name, phone } = payload;
        const wxContext = cloud.getWXContext();
        const openid = wxContext.OPENID;
        if (!openid) return { success: false, msg: '获取微信授权状态失败，请重试' };
        
        const existMatch = await db.collection('students').where({ name, phone }).get();
        if (!existMatch.data || existMatch.data.length === 0) {
          return { success: false, msg: '未找到匹配的学员，请核对家长手机号及孩子姓名' };
        }
        
        const studentId = existMatch.data[0]._id;
        await db.collection('students').doc(studentId).update({
          data: { _openid: openid }
        });
        
        return { success: true, msg: '接驳绑定成功' };
      }
      ...
      case 'submitNewApplication': {
        ...
        studentData._openid = openid;
        studentData.timestamp = Date.now();
        
        const addRes = await db.collection('students').add({ data: studentData });
        return { success: true, msg: '申请提交成功', id: addRes._id };
      }
